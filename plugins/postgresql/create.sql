...
begin
  ...
  if exists (select 1 from information_schema.tables where table_schema='metadata' and table_name='node_types') then
    ...
    perform metadata.add_nodes_type('postgresql/columns', 'select node_types.icon as "icon", node_types.type, columns.column_log as node_log, columns.column as node, columns.name, (''{}'')::jsonb as definition, ''{}''::jsonb as status, columns.observed_on, false as transient from postgresql.columns, metadata.node_types where node_types.name = ''postgresql/columns''');
    perform metadata.add_nodes_type('postgresql/constraints', 'select node_types.icon as "icon", node_types.type, constraints.constraint_log as node_log, constraints.constraint as node, constraints.name, (''{}'')::jsonb as definition, ''{}''::jsonb as status, constraints.observed_on, false as transient from postgresql.constraints, metadata.node_types where node_types.name = ''postgresql/constraints''');
    perform metadata.add_nodes_type('postgresql/databases', 'select node_types.icon as "icon", node_types.type, databases.database_log as node_log, databases.database as node, databases.host || ''/'' || databases.name as name, databases.config as definition, ''{}''::jsonb as status, databases.observed_on, false as transient from postgresql.databases, metadata.node_types where node_types.name = ''postgresql/databases''');
    perform metadata.add_nodes_type('postgresql/foreign_servers', 'select node_types.icon as "icon", node_types.type, foreign_servers.foreign_server_log as node_log, foreign_servers.foreign_server as node, foreign_servers.name, (''{}'')::jsonb as definition, ''{}''::jsonb as status, foreign_servers.observed_on, false as transient from postgresql.foreign_servers, metadata.node_types where node_types.name = ''postgresql/foreign_servers''');
    perform metadata.add_nodes_type('postgresql/indexes', 'select node_types.icon as "icon", node_types.type, indexes.index_log as node_log, indexes.index as node, indexes.name, (''{}'')::jsonb as definition, ''{}''::jsonb as status, indexes.observed_on, false as transient from postgresql.indexes, metadata.node_types where node_types.name = ''postgresql/indexes''');
    perform metadata.add_nodes_type('postgresql/roles', 'select node_types.icon as "icon", node_types.type, roles.role_log as node_log, roles.role as node, roles.username as name, (''{}'')::jsonb as definition, ''{}''::jsonb as status, roles.observed_on, false as transient from postgresql.roles, metadata.node_types where node_types.name = ''postgresql/roles''');
    perform metadata.add_nodes_type('postgresql/tables', 'select node_types.icon as "icon", node_types.type, tables.table_log as node_log, tables.table as node, tables.name, (''{}'')::jsonb as definition, ''{}''::jsonb as status, tables.observed_on, false as transient from postgresql.tables, metadata.node_types where node_types.name = ''postgresql/tables''');

    perform metadata.add_nodes_log_type('postgresql/columns', 'select node_types.icon as "icon", node_types.type, columns_log.column_log as node_log, columns_log.column as node, columns_log.name, (''{}'')::jsonb as definition, ''{}''::jsonb as status, columns_log.observed_on, false as transient, deleted from postgresql.columns_log, metadata.node_types where node_types.name = ''postgresql/columns''');
    perform metadata.add_nodes_log_type('postgresql/constraints', 'select node_types.icon as "icon", node_types.type, constraints_log.constraint_log as node_log, constraints_log.constraint as node, constraints_log.name, (''{}'')::jsonb as definition, ''{}''::jsonb as status, constraints_log.observed_on, false as transient, deleted from postgresql.constraints_log, metadata.node_types where node_types.name = ''postgresql/constraints''');
    perform metadata.add_nodes_log_type('postgresql/databases', 'select node_types.icon as "icon", node_types.type, databases_log.database_log as node_log, databases_log.database as node, databases_log.host || ''/'' || databases_log.name as name, databases_log.config as definition, ''{}''::jsonb as status, databases_log.observed_on, false as transient, deleted from postgresql.databases_log, metadata.node_types where node_types.name = ''postgresql/databases''');
    perform metadata.add_nodes_log_type('postgresql/foreign_servers', 'select node_types.icon as "icon", node_types.type, foreign_servers_log.foreign_server_log as node_log, foreign_servers_log.foreign_server as node, foreign_servers_log.name, (''{}'')::jsonb as definition, ''{}''::jsonb as status, foreign_servers_log.observed_on, false as transient, deleted from postgresql.foreign_servers_log, metadata.node_types where node_types.name = ''postgresql/foreign_servers''');
    perform metadata.add_nodes_log_type('postgresql/indexes', 'select node_types.icon as "icon", node_types.type, indexes_log.index_log as node_log, indexes_log.index as node, indexes_log.name, (''{}'')::jsonb as definition, ''{}''::jsonb as status, indexes_log.observed_on, false as transient, deleted from postgresql.indexes_log, metadata.node_types where node_types.name = ''postgresql/indexes''');
    perform metadata.add_nodes_log_type('postgresql/roles', 'select node_types.icon as "icon", node_types.type, roles_log.role_log as node_log, roles_log.role as node, roles_log.username as name, (''{}'')::jsonb as definition, ''{}''::jsonb as status, roles_log.observed_on, false as transient, deleted from postgresql.roles_log, metadata.node_types where node_types.name = ''postgresql/roles''');
    perform metadata.add_nodes_log_type('postgresql/tables', 'select node_types.icon as "icon", node_types.type, tables_log.table_log as node_log, tables_log.table as node, tables_log.name, (''{}'')::jsonb as definition, ''{}''::jsonb as status, tables_log.observed_on, false as transient, deleted from postgresql.tables_log, metadata.node_types where node_types.name = ''postgresql/tables''');
  end if;
end
$$;