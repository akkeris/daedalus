    ...
    if (pod.definition.metadata.ownerReferences) {
      await Promise.all(pod.definition.metadata.ownerReferences.map(async (ref) => {
        try {
          ...
          debug(`Error unable to add link for pod ${pod.pod} to replicaset ${`${ref.name}/${pod.definition.metadata.namespace} due to: ${e.message}`}`); // eslint-disable-line max-len,no-console
        }
      }));
    }
  }));
    ...
    if (replicaSet.definition.metadata.ownerReferences) {
      await Promise.all(replicaSet.definition.metadata.ownerReferences.map(async (ref) => {
        try {
          ...
          debug(`Error unable to add link for replicaset ${replicaSet.replicaset} to deployment ${`${ref.name}/${replicaSet.definition.metadata.namespace}`} due to ${e.message}`); // eslint-disable-line max-len,no-console
        }
      }));
    }
  }));
