...
describe('postgresql', async () => { // eslint-disable-line no-undef
  ...
  it('adding a database', async () => { // eslint-disable-line no-undef
    ...
    const { rows: [{ database_log }] } = await pgpool.query('insert into postgresql.databases_log (database_log, database, name, host, port, observed_on, deleted) values (uuid_generate_v4(), uuid_generate_v4(), $1, $2, $3, now(), false) returning database_log', // eslint-disable-line camelcase
      [connString.pathname.substring(1), connString.hostname, connString.port || 5432]);
    await pgpool.query('insert into postgresql.roles_log (role_log, role,  database_log, username, password, options, observed_on, deleted) values (uuid_generate_v4(), uuid_generate_v4(), $1, $2, $3, $4, now(), false)',
      [database_log, connString.username, security.encryptValue('ABCDEFABCDEFABCDEFABCDEF', connString.password), connString.search.replace(/\?/, '')]); // eslint-disable-line camelcase
    await postgresql.exec(pgpool, bus, 'ABCDEFABCDEFABCDEFABCDEF');
    bus.off('postgresql.error', testFailed);
    databaseUUID = database_log; // eslint-disable-line camelcase
  });
  ...
  it('checking tables view', async () => { // eslint-disable-line no-undef
    ...
    assert.equal(tables.filter((x) => x.schema === 'postgresql').length, 23);
    ...
    columnTableUUID = tables.filter((x) => x.name === 'columns_log' && !x.is_view)[0].table_log;
  });
  it('checking column view', async () => { // eslint-disable-line no-undef
    ...
    const columnTableColumns = columns.filter((x) => x.table_log === columnTableUUID && x.database_log === databaseUUID); // eslint-disable-line max-len
    assert.ok((new Date()).getTime() - columns[0].observed_on.getTime() < 1000 * 10);
    assert.equal(columnTableColumns.length, 20);
  });
  it('checking indexes view', async () => { // eslint-disable-line no-undef
    ...
    const columnTableIndexes = indexes.filter((x) => x.table_log === columnTableUUID && x.database_log === databaseUUID); // eslint-disable-line max-len
  ...
  it('checking constraints view (check)', async () => { // eslint-disable-line no-undef
    ...
    const tableConstraints = constraints.filter((x) => x.database_log === databaseUUID && x.from_schema === 'postgresql');
  ...
  it('checking constraints view (foreign key)', async () => { // eslint-disable-line no-undef
    ...
    const tableConstraints = constraints.filter((x) => x.database_log === databaseUUID && x.from_schema === 'postgresql');
    assert.ok((new Date()).getTime() - constraints[0].observed_on.getTime() < 1000 * 10);
    assert.equal(tableConstraints.filter((x) => x.name === 'columns_log_table_log_fkey' && x.type === 'FORIEGN KEY').length, 1);
  });
});
