...
module.exports = async function addExpressRoutes(pgpool, bus, app) {
  ...
  app.get('/ui/oracle/databases/:oracle_database_id', async (req, res, next) => {
    ...
    const { rows: indexes } = await pgpool.query('select * from oracle.indexes where database = $1', [req.params.oracle_database_id]);
    const { rows: constraints } = await pgpool.query('select * from oracle.constraints where database = $1', [req.params.oracle_database_id]);
    const { rows: databaseStatistics } = await pgpool.query('select * from oracle.database_statistics where database = $1', [req.params.oracle_database_id]);
    const { rows: tableStatistics } = await pgpool.query('select * from oracle.table_statistics where database = $1', [req.params.oracle_database_id]);
    ...
    const data = {
      ...
      users: await usersAndWatchers(pgpool, req.params.oracle_database_id),
